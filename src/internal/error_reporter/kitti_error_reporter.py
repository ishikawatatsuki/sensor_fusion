...
def evaluate_kitti_odom(gt_pose: np.ndarray, estimated_pose: np.ndarray):
    """
    Evaluate KITTI odometry using ATE and RPE.

    Args:
        gt_pose (np.ndarray): Ground truth poses, shape (N, 3, 4)
        estimated_pose (np.ndarray): Estimated poses, shape (N, 3, 4)

    Returns:
        ate (float): Absolute trajectory error (RMSE in meters)
        rpe_trans (float): Relative pose error (mean translation in meters)
        rpe_rot_deg (float): Relative pose error (mean rotation in degrees)
    """
    assert gt_pose.shape == estimated_pose.shape
    N = gt_pose.shape[0]
    ...
    def compute_ATE(gt_T, est_T):
        errors = []
        for i in range(N):
            err = gt_T[i][:3, 3] - est_T[i][:3, 3]
            errors.append(np.linalg.norm(err))
        return np.sqrt(np.mean(np.array(errors) ** 2))

    def compute_RPE(gt_T, est_T):
        trans_errors = []
        rot_errors = []
        for i in range(N - 1):
            gt_rel = np.linalg.inv(gt_T[i]) @ gt_T[i + 1]
            est_rel = np.linalg.inv(est_T[i]) @ est_T[i + 1]
            rel_err = np.linalg.inv(gt_rel) @ est_rel
    ...
    # Convert 3x4 -> 4x4 matrices
    def to_homogeneous(pose_3x4):
        T = np.eye(4)
        T[:3, :4] = pose_3x4
        return T

    gt_T = np.array([to_homogeneous(p) for p in gt_pose])
    est_T = np.array([to_homogeneous(p) for p in estimated_pose])

    # Align both to the first frame
    gt_T0_inv = np.linalg.inv(gt_T[0])
    est_T0_inv = np.linalg.inv(est_T[0])
    gt_T = np.array([gt_T0_inv @ p for p in gt_T])
    est_T = np.array([est_T0_inv @ p for p in est_T])

    # Optional: scale optimization
    gt_xyz = np.stack([T[:3, 3] for T in gt_T])
    est_xyz = np.stack([T[:3, 3] for T in est_T])
    scale = scale_lse_solver(est_xyz, gt_xyz)
    for i in range(N):
        est_T[i][:3, 3] *= scale

    ate = compute_ATE(gt_T, est_T)
    rpe_trans, rpe_rot_rad = compute_RPE(gt_T, est_T)
    rpe_rot_deg = rpe_rot_rad * 180.0 / np.pi

    return ate, rpe_trans, rpe_rot_deg
...
if __name__ == "__main__":
    ...
    ate, rpe_trans, rpe_rot_deg = evaluate_kitti_odom(gt_pose, estimated_pose)
    print(f"ATE: {ate:.3f} m, RPE: {rpe_trans:.3f} m, {rpe_rot_deg:.3f} deg")



    gt_positions = gt_pose[:, :3, 3]
    estimated_positions = estimated_pose[:, :3, 3]
    vo_positions = None
    ...
    ax.grid()
    plt.axis('equal')
    plt.savefig(f"./test_trajectory.png")
    plt.close(fig)