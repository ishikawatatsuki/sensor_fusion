...
from .mics import save_trajectory_plots, Result

DATASET_CONFIG_MAPS = {
    "imu_vo": "configs/.experiments/kitti_all_filters/seq07/kitti_dataset_config_imu_vo.yaml",
    "imu_vo_gps": "configs/.experiments/kitti_all_filters/seq07/kitti_dataset_config_imu_vo_gps.yaml"
}
...
def create_config(
    ...
    dataset_config_path = DATASET_CONFIG_MAPS[sensor]
...
def validate_config(
        experiment_config: ExtendedConfig, 
        expected_filter_type: str, 
        expected_sequence: str, 
        expected_sensor: str
        ):
    # Check dataset sequence
    if experiment_config.dataset.variant == expected_sequence:
        print(f"✓ Dataset sequence: {experiment_config.dataset.variant} == {expected_sequence}")
    else:
        logger.error(f"✗ Dataset sequence mismatch: {experiment_config.dataset.variant} != {expected_sequence}")
        raise ValueError(f"Dataset sequence mismatch: expected {expected_sequence}, got {experiment_config.dataset.variant}")
    
    # Check filter type
    if experiment_config.filter.type == expected_filter_type:
        print(f"✓ Filter type: {experiment_config.filter.type} == {expected_filter_type}")
    else:
        logger.error(f"✗ Filter type mismatch: {experiment_config.filter.type} != {expected_filter_type}")
        raise ValueError(f"Filter type mismatch: expected {expected_filter_type}, got {experiment_config.filter.type}")
    
    # Check motion model
    if experiment_config.filter.motion_model == "velocity":
        print(f"✓ Motion model: {experiment_config.filter.motion_model} == velocity")
    else:
        logger.error(f"✗ Motion model mismatch: {experiment_config.filter.motion_model} != velocity")
        raise ValueError(f"Motion model mismatch: expected velocity, got {experiment_config.filter.motion_model}")
    
    # Check sensor configuration by inspecting selected sensors
    selected_sensors = [s.name for s in experiment_config.dataset.sensors]
    has_gps = any('gps' in s_name.lower() for s_name in selected_sensors)
    
    if expected_sensor == "imu_vo":
        if not has_gps and any('imu' in s.lower() for s in selected_sensors) and any('vo' in s.lower() or 'stereo' in s.lower() for s in selected_sensors):
            print(f"✓ Sensor configuration: {expected_sensor} (sensors: {selected_sensors})")
        else:
            logger.error(f"✗ Sensor configuration mismatch: expected {expected_sensor}, got {selected_sensors}")
            raise ValueError(f"Sensor configuration mismatch: expected {expected_sensor} without GPS")
    elif expected_sensor == "imu_vo_gps":
        if has_gps and any('imu' in s.lower() for s in selected_sensors) and any('vo' in s.lower() or 'stereo' in s.lower() for s in selected_sensors):
            print(f"✓ Sensor configuration: {expected_sensor} (sensors: {selected_sensors})")
        else:
            logger.error(f"✗ Sensor configuration mismatch: expected {expected_sensor}, got {selected_sensors}")
            raise ValueError(f"Sensor configuration mismatch: expected {expected_sensor} with GPS")
    else:
        logger.warning(f"⚠ Unknown sensor configuration: {expected_sensor}")
        print(f"⚠ Sensor configuration: {expected_sensor} (sensors: {selected_sensors})")
...
if __name__ == "__main__":
    ...
    config_file_path = "./configs/.experiments/kitti_all_filters/seq07/all_filters_config.yaml"
    ...
    logging.info("----- Starting KITTI experiments with different configurations -----")
    for seq, filter_type, sensor in tqdm(product(sequences, filter_types, sensors)):
        ...
        # Validate experiment configuration matches expected setup
        # print("----- Validating Experiment Configuration -----")
        # validate_config(
        #     experiment_config=experiment_config,
        #     expected_filter_type=filter_type,
