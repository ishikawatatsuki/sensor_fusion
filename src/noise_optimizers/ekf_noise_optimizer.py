...
class EKF_NoiseOptimizer:
  ...
  ekf_1 = None
  ekf_1_optimized = None

  ekf_2 = None
  ekf_2_optimized = None

  ekf_3 = None
  ekf_3_optimized = None
  ...
  def run(self):
    ...
    self.x, self.P, self.H, q1, r_vo1, r_gps1 = self.data.get_initial_data(setup=self.setup, filter_type=FilterEnum.EKF)
    ...
    self.x, self.P, self.H, q2, r_vo2, r_gps2 = self.data.get_initial_data(setup=self.setup, filter_type=FilterEnum.EKF)
    ...
    self.x, self.P, self.H, q3, r_vo3, r_gps3 = self.data.get_initial_data(setup=self.setup, filter_type=FilterEnum.EKF)
  ...
  def compare(self, load_exported=False):
    ...
    error_2 = self.ekf_2.run(data=self.data)
    ...
    error_3 = self.ekf_3.run(data=self.data)
  ...
  def visualize_results(self):
    ...
    ax1.plot(self.ekf_1.mu_x, self.ekf_1.mu_y, lw=2, label='Estimated result (non-optimal)', color='b')
    ax1.plot(self.ekf_1_optimized.mu_x, self.ekf_1_optimized.mu_y, lw=2, label='Estimated result (optimal)', color='r')
    ax1.set_xlabel('X [m]')
    ax1.set_ylabel('Y [m]')
    ...
    ax2.plot(self.ekf_2.mu_x, self.ekf_2.mu_y, lw=2, label='Estimated result (non-optimal)', color='b')
    ax2.plot(self.ekf_2_optimized.mu_x, self.ekf_2_optimized.mu_y, lw=2, label='Estimated result (optimal)', color='r')
    ax2.set_xlabel('X [m]')
    ax2.set_ylabel('Y [m]')
    ...
    ax3.plot(self.ekf_3.mu_x, self.ekf_3.mu_y, lw=2, label='Estimated result (non-optimal)', color='b')
    ax3.plot(self.ekf_3_optimized.mu_x, self.ekf_3_optimized.mu_y, lw=2, label='Estimated result (optimal)', color='r')
    ax3.set_xlabel('X [m]')
    ax3.set_ylabel('Y [m]')
