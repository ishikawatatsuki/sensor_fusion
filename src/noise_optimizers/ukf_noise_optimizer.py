...
class UKF_NoiseOptimizer:
  ...
  ukf_1 = None
  ukf_1_optimized = None

  ukf_2 = None
  ukf_2_optimized = None

  ukf_3 = None
  ukf_3_optimized = None
  ...
  def run(self):
    ...
    print(ukf_params[self.setup])
    self.x, self.P, self.H, q1, r_vo1, r_gps1 = self.data.get_initial_data(
      setup=self.setup, 
    ...
    self.x, self.P, self.H, q2, r_vo2, r_gps2 = self.data.get_initial_data(
      setup=self.setup, 
      filter_type=FilterEnum.UKF)
    ...
    self.x, self.P, self.H, q3, r_vo3, r_gps3 = self.data.get_initial_data(
      setup=self.setup, 
      filter_type=FilterEnum.UKF)
  ...
  def visualize_results(self):
    ...
    ax1.plot(self.ukf_1.mu_x, self.ukf_1.mu_y, lw=2, label='Estimated result (non-optimal)', color='b')
    ax1.plot(self.ukf_1_optimized.mu_x, self.ukf_1_optimized.mu_y, lw=2, label='Estimated result (optimal)', color='r')
    ax1.set_xlabel('X [m]')
    ax1.set_ylabel('Y [m]')
    ...
    ax2.plot(self.ukf_2.mu_x, self.ukf_2.mu_y, lw=2, label='Estimated result (non-optimal)', color='b')
    ax2.plot(self.ukf_2_optimized.mu_x, self.ukf_2_optimized.mu_y, lw=2, label='Estimated result (optimal)', color='r')
    ax2.set_xlabel('X [m]')
    ax2.set_ylabel('Y [m]')
    ...
    ax3.plot(self.ukf_3.mu_x, self.ukf_3.mu_y, lw=2, label='Estimated result (non-optimal)', color='b')
    ax3.plot(self.ukf_3_optimized.mu_x, self.ukf_3_optimized.mu_y, lw=2, label='Estimated result (optimal)', color='r')
    ax3.set_xlabel('X [m]')
    ax3.set_ylabel('Y [m]')
