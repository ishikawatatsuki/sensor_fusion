...
Currently, all the optimizer python scripts refer to the KITTI example dataset stored in `example_data/KITTI/` so that no data download is needed. The result is stored in temporary folder labeled by `{filter name}_example` under the same export path `exports/_noise_optimizations/`.
