...
class CKF_NoiseOptimizer:
  ...
  ckf_1 = None
  ckf_1_optimized = None

  ckf_2 = None
  ckf_2_optimized = None

  ckf_3 = None
  ckf_3_optimized = None
  ...
  def run(self):
    ...
    self.x, self.P, self.H, q1, r_vo1, r_gps1 = self.data.get_initial_data(setup=self.setup, filter_type=FilterEnum.CKF)
    x0_1 = np.concatenate([q1, r_vo1, r_gps1]).tolist()
    self.result_1 = minimize(self.J, x0_1, method="nelder-mead")
    print(self.result_1)

    print("For setup 2")
    self.setup = SetupEnum.SETUP_2
    self.x, self.P, self.H, q2, r_vo2, r_gps2 = self.data.get_initial_data(setup=self.setup, filter_type=FilterEnum.CKF)
    x0_2 = np.concatenate([q2, r_vo2, r_gps2]).tolist()
    self.result_2 = minimize(self.J, x0_2, method="nelder-mead")
    print(self.result_2)
    
    print("For setup 3")
    self.setup = SetupEnum.SETUP_3
    self.x, self.P, self.H, q3, r_vo3, r_gps3 = self.data.get_initial_data(setup=self.setup, filter_type=FilterEnum.CKF)
    x0_3 = np.concatenate([q3, r_vo3, r_gps3]).tolist()
    self.result_3 = minimize(self.J, x0_3, method="nelder-mead")
    print(self.result_3)
  ...
  def compare(self, load_exported=False):
    x_1, P_1, H_1, q1, r_vo1, r_gps1 = self.data.get_initial_data(setup=SetupEnum.SETUP_1, filter_type=FilterEnum.CKF, noise_type=NoiseTypeEnum.CURRENT)

    self.ckf_1 = CubatureKalmanFilter(
      x=x_1.copy(), 
      P=P_1.copy(), 
      H=H_1.copy(),
      q=q1,
    ...
    error_1 = self.ckf_1.run(data=self.data)
    ...
    self.ckf_1_optimized = CubatureKalmanFilter(
      x=x_1.copy(), 
      P=P_1.copy(), 
      H=H_1.copy(),
      q=q1_optimal,
    ...
    error_1_optimized = self.ckf_1_optimized.run(data=self.data)




    x_2, P_2, H_2, q2, r_vo2, r_gps2 = self.data.get_initial_data(setup=SetupEnum.SETUP_2, filter_type=FilterEnum.CKF, noise_type=NoiseTypeEnum.CURRENT)
    
    self.ckf_2 = CubatureKalmanFilter(
      x=x_2.copy(), 
      P=P_2.copy(), 
      H=H_2.copy(),
      q=q2,
    ...
    error_2 = self.ckf_2.run(data=self.data)
    ...
    self.ckf_2_optimized = CubatureKalmanFilter(
      x=x_2.copy(), 
      P=P_2.copy(), 
      H=H_2.copy(),
      q=q2_optimal,
    ...
    error_2_optimized = self.ckf_2_optimized.run(data=self.data)




    x_3, P_3, H_3, q3, r_vo3, r_gps3 = self.data.get_initial_data(setup=SetupEnum.SETUP_3, filter_type=FilterEnum.CKF, noise_type=NoiseTypeEnum.CURRENT)

    self.ckf_3 = CubatureKalmanFilter(
      x=x_3.copy(), 
      P=P_3.copy(), 
      H=H_3.copy(),
      q=q3,
    ...
    error_3 = self.ckf_3.run(data=self.data)
    ...
    self.ckf_3_optimized = CubatureKalmanFilter(
      x=x_3.copy(), 
      P=P_3.copy(), 
      H=H_3.copy(),
      q=q3_optimal,
    ...
    error_3_optimized = self.ckf_3_optimized.run(data=self.data)
  ...
  def visualize_results(self):
    ...
    ax1.plot(self.ckf_1.mu_x, self.ckf_1.mu_y, lw=2, label='Estimated result (non-optimal)', color='b')
    ax1.plot(self.ckf_1_optimized.mu_x, self.ckf_1_optimized.mu_y, lw=2, label='Estimated result (optimal)', color='r')
    ax1.set_xlabel('X [m]')
    ax1.set_ylabel('Y [m]')
    ...
    ax2.plot(self.ckf_2.mu_x, self.ckf_2.mu_y, lw=2, label='Estimated result (non-optimal)', color='b')
    ax2.plot(self.ckf_2_optimized.mu_x, self.ckf_2_optimized.mu_y, lw=2, label='Estimated result (optimal)', color='r')
    ax2.set_xlabel('X [m]')
    ax2.set_ylabel('Y [m]')
    ...
    ax3.plot(self.ckf_3.mu_x, self.ckf_3.mu_y, lw=2, label='Estimated result (non-optimal)', color='b')
    ax3.plot(self.ckf_3_optimized.mu_x, self.ckf_3_optimized.mu_y, lw=2, label='Estimated result (optimal)', color='r')
    ax3.set_xlabel('X [m]')
    ax3.set_ylabel('Y [m]')
